...
class PdfJavaScriptDetectorTest extends TestCase
{
    ...
    public function testPdfJavaScriptDetection(string $filename, bool $expected): void
    {
        ...
        try {
            ...
            $this->fail("Exception occurred while testing {$filename}: " . $e->getMessage());
        } catch (\Throwable $e) {
    ...
    public function testDetectJavaScriptWithNonExistentFile(): void
    {
        $this->expectException(\InvalidArgumentException::class);
        $this->detector->detectJavaScript('non-existent-file.pdf');
    }

    public function testDetectJavaScriptWithEmptyFile(): void
    {
        $emptyFile = tempnam(sys_get_temp_dir(), 'test_empty_');
        file_put_contents($emptyFile, '');

        try {
            $result = $this->detector->detectJavaScript($emptyFile);
            $this->assertFalse($result, 'Empty file should not contain JavaScript');
        } finally {
            unlink($emptyFile);
        }
    }
